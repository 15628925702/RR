...
ap = argparse.ArgumentParser(); ap.add_argument('--budget',type=int,required=True); ap.add_argument('--rep',type=int,required=True); ap.add_argument('--h-tilted',type=int,required=True); ap.add_argument('--h-cond',type=int,required=True); ap.add_argument('--qmc-order',type=int,default=8); ap.add_argument('--out',required=True); args=ap.parse_args()
try:
    m=make_frozen_mixture(seed=2026,alpha=1.0); sc=reference_scale(m,6000,2026); p=pickle.load(open('experiments/p4_prepared_oracle_hp.pkl','rb')); t=time.time(); seed=202600000+args.budget*1000+args.rep
    r=run_replication(m,sc,all_pairs(),args.budget,seed,prepared=p,lu=args.budget,h_tilted=args.h_tilted,h_cond=args.h_cond,kl_samples=512,pilot_norm_cap=None,scoring_steps=2,theta_norm_cap=4.0,theta_l1_cap=5.0,kl_mu_direct=False,use_oracle_H=False,policies=['oracle RR-GID'],conditional_method='qmc',qmc_order=args.qmc_order)[0]
